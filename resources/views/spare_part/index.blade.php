<x-layout.default>
    <div x-data="striped" class="panel">
        <div class="flex justify-between mb-6">
            <h5 class="font-semibold text-lg dark:text-white-light">Suku Cadang</h5>
            <a href="{{ route('spare_part.create') }}" class="btn btn-primary">Tambah</a>
        </div>
    ...
    <script>
        document.addEventListener("alpine:init", () => {
            Alpine.data("striped", () => ({
                spareParts: @json($spareParts->toArray()),
                dataTable: null,
                init() {
                    this.initDataTable();
                },
                initDataTable() {
                    const sparePartEditUrl = "{{ route('spare_part.edit', ['sparePart' => ':id']) }}";
                    const sparePartDestroyUrl =
                        "{{ route('spare_part.destroy', ['sparePart' => ':id']) }}";
                    const tableOptions = {
                        data: {
                            headings: ["ID", "Nama", "Stok", "Harga", "Aksi"],
                            data: this.spareParts.map((sparePart, index) => {
                                const editSparePart = sparePartEditUrl.replace(':id',
                                    sparePart.id);
                                const destroySparePart = sparePartDestroyUrl.replace(':id',
                                    sparePart.id);
                                return [
                                    index + 1,
                                    sparePart.name,
                                    sparePart.quantity,
                                    `Rp ${sparePart.price.toLocaleString('id-ID', { minimumFractionDigits: 2 })}`,
                                    `<div class="flex items-center gap-2">
                                        <a href="${editSparePart}">
                                        ...
                                        <button @click.prevent="confirmDelete(${sparePart.id})" type="button">
                                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                async confirmDelete(sparePartId) {
                    const isConfirmed = await this.showConfirmDialog();
                    if (isConfirmed) {
                        await this.deleteSparePart(sparePartId);
                    }
                },
                async deleteSparePart(sparePartId) {
                    const csrfToken = document.querySelector('meta[name="csrf-token"]')
                        .getAttribute('content');
                    const rute = "{{ route('spare_part.destroy', ':id') }}";
                    const url = rute.replace(':id', sparePartId);
                    try {
                ...
                removeSparePartFromList(sparePartId) {
                    const index = this.spareParts.findIndex(data => data.id === sparePartId);
                    if (index !== -1) {
